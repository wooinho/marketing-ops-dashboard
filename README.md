...
| 4. AI 챗봇 *(파일럿, 비밀번호 보호 없음)* | Claude API 기반 업무 도우미. 노션·슬랙·구글드라이브·그룹메일 연동 |
...
이 대시보드는 **Public**(공개) 저장소/앱입니다. 챗봇은 내부 자료(노션·슬랙·구글드라이브·
...
GMAIL_DELEGATED_USER = "group@wylie.co.kr"  # 아래 "그룹메일 도메인 전체 위임" 참고

[google_service_account]            # 아래 "구글드라이브 서비스 계정 만들기" 참고
# JSON 키 파일 내용 그대로 TOML 테이블로 옮겨 붙여넣기 (자세한 형식은 .streamlit/secrets.toml.example)
# 구글드라이브와 그룹메일 모두 이 서비스 계정 하나를 공유해서 씁니다.
```

`NOTION_TOKEN`/`SLACK_BOT_TOKEN`/`[google_service_account]`/`GMAIL_DELEGATED_USER`가 없으면
해당 소스 없이 일반 대화만 가능하고, `ANTHROPIC_API_KEY`가 없으면 페이지 전체가 "설정 필요"
안내만 표시합니다. (그룹메일은 `[google_service_account]`와 `GMAIL_DELEGATED_USER`가 **둘 다**
있어야 활성화됩니다.)
...
### 그룹메일 도메인 전체 위임 (GMAIL_DELEGATED_USER)

**⚠️ 이 단계는 Google Workspace 관리자 권한이 필요합니다.** 구글드라이브 서비스 계정과
달리, Gmail은 파일 공유만으로 접근할 수 없고 관리자가 "이 서비스 계정이 특정 사서함인 척
행동해도 된다"고 명시적으로 승인해야 합니다.

1. 위에서 만든 서비스 계정 상세 페이지 → **고유 ID(클라이언트 ID)** 복사 (client_email이 아닌
   숫자로 된 고유 ID입니다)
2. https://console.cloud.google.com → **API 및 서비스 → 라이브러리** → **Gmail API** 검색 → 사용
3. **admin.google.com** (Google Workspace 관리 콘솔) → **보안 → 액세스 및 데이터 제어 →
   API 제어 → 도메인 전체 위임 관리** → **"새로 추가"**
4. 1번에서 복사한 클라이언트 ID 입력, OAuth 범위에
   `https://www.googleapis.com/auth/gmail.readonly` 입력 → 승인
5. `GMAIL_DELEGATED_USER`에는 위임 대상 사서함 이메일을 입력합니다 — 그룹메일 주소
   자체(예: `group@wylie.co.kr`)로 되는 경우도 있고, 안 되면 그 그룹메일에 접근 권한이
   있는 특정 팀원의 개인 계정 이메일을 대신 지정해야 할 수 있습니다(환경에 따라 다름).

> ⚠️ 그룹메일도 **전체 메일함 검색이 아닙니다.** 위임 설정된 사서함 한 곳만 검색됩니다.

**연동 로드맵**: 노션 → 슬랙 → 구글드라이브 → 그룹메일(코드 완료, 관리자 설정 대기) —
4개 소스 모두 동일한 패턴(검색 → 텍스트 추출 → `src/chatbot.py`의 컨텍스트에 추가)을 따릅니다.
