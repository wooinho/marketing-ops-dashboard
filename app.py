...
from src import app_state, database as db, metrics, transformation
...
st.divider()
st.header("본부 전체 현황 (YTD)")

annual = app_state.load_table("revenue_group_annual")
if annual.empty:
    st.info("적재된 데이터가 없습니다. 사이드바 메뉴에서 데이터를 확인해주세요.")
else:
    total = annual[annual["grp"] == "전체"].iloc[0]
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("목표매출(연간)", metrics.format_currency(total["revenue_target"]))
    c2.metric("발생매출(YTD)", metrics.format_currency(total["revenue_actual"]),
               delta=metrics.format_percent(total["revenue_rate_pct"]))
    c3.metric("목표매출이익(연간)", metrics.format_currency(total["profit_target"]))
    c4.metric("발생매출이익(YTD)", metrics.format_currency(total["profit_actual"]),
               delta=metrics.format_percent(total["profit_rate_pct"]))

    st.caption(
        "데이터 기준: 1그룹은 11월까지, 2그룹은 10월까지 실적이 반영되어 있습니다. "
        "12월 등 미래월은 목표만 존재하며 실적은 아직 없습니다."
    )
...
st.markdown(
    ...
    1. **본부 전체 KPI** — 선택월 목표 대비 실적(매출/매출이익) 및 YTD 월별 추이
    2. **1그룹 vs 2그룹 비교** — 그룹별 매출/매출이익 비교, 광고주별 상세 테이블
    3. **그룹별 MM 생산성** *(파일럿 — 수식 미검증)* — 클라이언트/담당자별 MM 투입 및 생산성
    4. **그룹별 담당자 프로젝트 현황** — 담당자 × 클라이언트 역할(PM/S.AE/AE) 매트릭스
    5. **AI 챗봇** *(파일럿 — 비밀번호 보호, 현재 노션·슬랙 연동)* — 논의·결정사항 요약, 문서 검색, 업무 리마인드
    """
)
...
with st.expander("⚠️ 알려진 데이터 이슈 펼쳐보기"):
    st.markdown(
        ...
        - **스냅샷 선택**: 매출 트래킹 시트의 "마캠그룹 전체" 표는 시트 내에 두 번(스냅샷 A/B) 등장합니다.
          이 대시보드는 더 최신이고 미래월이 올바르게 0으로 처리된 **스냅샷 B**(YTD 매출 ₩5,717.8M)를 정본으로 사용합니다.
